...
import com.example.cryptoapp.HomeFragmentDirections
import com.example.cryptoapp.MarketFragment
import com.example.cryptoapp.MarketFragmentDirections
...
class MarketAdapter(var context: Context, var list: List<CryptoCurrency>, var type: String) : RecyclerView.Adapter<MarketAdapter.MarketViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MarketViewHolder, position: Int) {
        ...
        if (item.quotes!![0].percentChange24h>0){
            ...
            holder.binding.currencyChangeTextView.setTextColor(context.resources.getColor(R.color.crimson))
            holder.binding.currencyChangeTextView.text = "${String.format("%.02f", item.quotes[0].percentChange24h)} %"
